      ...
      <Panel title="دفتر ديون الزبائن" description="الأرصدة المفتوحة وحركة المبيعات لكل زبون.">
        <SimpleRows
          ...
          actions={data.customers.map((customer) => (
            <div key={customer.id} className="flex flex-wrap items-center justify-end gap-2">
              <Link href={`/customers/${customer.id}`} className="inline-flex min-h-9 items-center rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-sm font-semibold text-cyan-800 transition hover:bg-cyan-100">
                فتح الحساب
              </Link>
              <RowActions onEdit={() => openEdit(customer)} onDelete={() => remove(customer)} editLoading={saving === `customer-${customer.id}`} deleteLoading={saving === `delete-customer-${customer.id}`} />
            </div>
          ))}
